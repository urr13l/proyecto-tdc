...
if (isset($_POST["actionInicio"])) {
  ...
  $consulta_query = mysqli_query($conexion, "SELECT c_usuario FROM cliente WHERE c_usuario = '$usuarioInicio' AND c_contrasenia = '$contraInicio'") or die("No se pudo realizar la consulta");
  $buscar_query = mysqli_num_rows($consulta_query);
  ...
  if ($buscar_query != 0) {
    ...
    if ($buscar_contra != 0) {
      ...
      $_SESSION['usuarioactua'] = $usuarioInicio;
      header("location:perfilSesion.php");
      echo "<script>alert('Usuario y contraseña validos');
                window.location.href = \"perfilSesion.php\"</script>";
    } else {
      mysqli_close($conexion);
